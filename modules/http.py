...
class _PinnedIPAdapter(HTTPAdapter):
    """
    Transport adapter that connects to a pre-validated IP address while keeping
    the TLS SNI and certificate validation bound to the ORIGINAL hostname.

    This closes the DNS-rebinding / TOCTOU gap: the IP we validated against the
    blocked-network list is the exact IP the socket connects to, instead of
    letting ``requests`` perform a second, unvalidated DNS lookup at connect
    time (which an attacker-controlled resolver could point at 169.254.169.254
    or 127.0.0.1). HTTPS certificates are still verified against the original
    hostname, so pinning does not weaken TLS.
    """
    ...
    def send(self, request, **kwargs):
        ...
        # Preserve the Host the origin server expects (host[:port], no userinfo).
        host_header = parsed.hostname
        if parsed.port:
            host_header = f"{host_header}:{parsed.port}"
        request.headers["Host"] = host_header

        # For HTTPS, pin SNI + certificate validation to the original hostname
        # even though the TCP connection targets the raw IP.
        if parsed.scheme.lower() == "https":
            self.poolmanager.connection_pool_kw["server_hostname"] = self._pinned_hostname
            self.poolmanager.connection_pool_kw["assert_hostname"] = self._pinned_hostname

        # Rewrite the URL so the actual connection targets the validated IP.
        if ":" in self._pinned_ip and not self._pinned_ip.startswith("["):
            ip_host = f"[{self._pinned_ip}]"  # bracket IPv6 literals
        else:
            ip_host = self._pinned_ip
        ip_netloc = f"{ip_host}:{parsed.port}" if parsed.port else ip_host
        request.url = urlunparse(parsed._replace(netloc=ip_netloc))

        resp = super().send(request, **kwargs)
        # Restore the original (hostname) URL on the response so callers and
        # logs never see the pinned IP, and redirect resolution stays correct.
...
class PromptInjectionDetector:
    ...
    HIGH_RISK_PATTERNS = [
        ...
        (
            r"(?i)\bignore\s+(?:all\s+)?(?:previous|above|prior|earlier|existing)\s+(?:instructions|rules|guidelines|prompts|commands|directives)\b",
            "OVERRIDE_DIRECTIVE",
        ),
        (
            r"(?i)\bdisregard\s+(?:all\s+)?(?:previous|above|prior|existing)\s+(?:instructions|rules|guidelines|prompts)\b",
            "OVERRIDE_DIRECTIVE",
        ),
        (
            r"(?i)\bforget\s+(?:all\s+)?(?:previous|above|prior|existing)\s+(?:instructions|rules|guidelines|prompts|your\s+role)\b",
            "FORGET_INSTRUCTION",
        ),
        (
            r"(?i)\boverride\s+(?:all\s+)?(?:previous|above|prior|existing)\s+(?:instructions|rules|guidelines|prompts|system|security)\b",
            "OVERRIDE_DIRECTIVE",
        ),
        (
            r"(?i)\bskip\s+(?:all\s+)?(?:previous|above|prior|existing)\s+(?:instructions|rules|guidelines|prompts|filters|safety)\b",
            "OVERRIDE_DIRECTIVE",
        ),
        (
            r"(?i)\bbypass\s+(?:all\s+)?(?:previous|above|prior|existing)\s+(?:instructions|rules|guidelines|prompts|filters|safety)\b",
            "OVERRIDE_DIRECTIVE",
        ),
        # Role/persona hijacking
        (
            r"(?i)\b(?:act\s+as|pretend\s+to\s+be|assume\s+the\s+role|take\s+on\s+the\s+role|play\s+the\s+role)\s+(?:of\s+)?(?:\w+\s+)*(?:developer|admin|system|programmer|coder|assistant|chatbot|bot|ai|language\s+model)\b",
            "ROLE_HIJACK",
        ),
        (
            r"(?i)\byou\s+are\s+(?:now|to\s+be|starting\s+now|from\s+now\s+on)\s+(?:a\s+)?(?:\w+\s+)*(?:developer|admin|system|programmer|coder|assistant|chatbot|bot|ai|language\s+model)\b",
            "ROLE_HIJACK",
        ),
        (
            r"(?i)\b(?:your\s+)?(?:new\s+)?(?:role|identity|persona|character)\s+(?:is|becomes|changes\s+to)\s+(?:a\s+)?(?:\w+\s+)*(?:developer|admin|system|programmer|coder|assistant|chatbot|bot|ai)\b",
            "ROLE_HIJACK",
        ),
        # System prompt extraction
        (
            r"(?i)\b(?:print|output|reveal|show|display|return|list|dump|extract|output|display)\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions|instructions|system\s+prompt|initial\s+instructions|developer\s+instructions|internal\s+instructions|prompt\s+instructions)\b",
            "SYSTEM_EXTRACT",
        ),
        (
            r"(?i)\b(?:repeat|say|write|print|output|display)\s+(?:exactly|verbatim|word\s+for\s+word|exactly\s+what|the\s+exact)\s+(?:your\s+)?(?:system\s+)?(?:prompt|instructions|rules|guidelines)\b",
            "SYSTEM_EXTRACT",
        ),
        # Mode switching
        (
            r"(?i)\b(?:enter|switch\s+to|activate|enable|go\s+into)\s+(?:\w+\s+)*(?:debug|developer|admin|system|unrestricted|free\s+mode|developer\s+mode|playground|test\s+mode|jailbreak|dan\s+mode|god\s+mode|evil\s+mode)\b",
            "MODE_SWITCH",
        ),
        (
            r"(?i)\b(?:you\s+are\s+now|you\s+have\s+been|you\s+are\s+going\s+to\s+be)\s+(?:a\s+)?(?:\w+\s+)*(?:developer|admin|system|unrestricted|free\s+mode|developer\s+mode|playground|test\s+mode|jailbreak|dan\s+mode|god\s+mode)\b",
            "MODE_SWITCH",
        ),
        # Instruction smuggling via translation/summarization
        (
            r"(?i)\b(?:translate|summarize|repeat|output|translate|convert|transliterate)\b.*?\b(?:but\s+first|and\s+then|also|however|instead|before|after)\b.*?\b(?:ignore|forget|disregard|override|bypass|skip)\b",
            "INSTRUCTION_SMUGGLE",
        ),
        (
        ...
        (
            r"(?i)\b(?:ignorer|ignorez|oublier|ignori|ignora|olvide|ignorar|ignorar|ignorer|无視|無視|すべて|すべてを|無視し|すべての|無視)\b",
            "MULTILINGUAL_ATTACK",
        ),
        # Encoding/obfuscation attempts
        (
            r"(?i)\b(?:decode|decrypt|unbase64|unhex|unencode|deobfuscate|disassemble)\s+(?:this|the\s+following|the\s+below)\s+(?:text|string|content|payload)\b",
            "ENCODED_INJECTION",
        ),
        # API/tool manipulation
        (
            r"(?i)\b(?:disable|turn\s+off|deactivate|remove|delete|remove)\s+(?:all\s+)?(?:filters|restrictions|limitations|constraints|safety\s+measures|guardrails|protocols|checks)\b",
            "DISARM_CONTROLS",
        ),
        (
            r"(?i)\b(?:allow|enable|activate|grant\s+permission)\s+(?:me\s+to\s+)?(?:do\s+anything|access\s+everything|perform\s+any\s+action|execute\s+any\s+command)\b",
            "GRANT_UNLIMITED",
        ),
    ]
    ...
    def detect_injection(cls, content: str) -> dict:
        ...
        if "SYSTEM_EXTRACT" in patterns_found or "MULTILINGUAL_ATTACK" in patterns_found:
            ...
            "OVERRIDE_DIRECTIVE" in patterns_found
            or "ROLE_HIJACK" in patterns_found
            or "MODE_SWITCH" in patterns_found
            or len(patterns_found) >= 3
        ):
            risk_level = "high"
        elif len(patterns_found) >= 2 or len(patterns_found) >= 1:
            risk_level = "medium"
        else:
            risk_level = "low"
    ...
    def _detect_encoded_injection(cls, content: str) -> bool:
        ...
        # URL-safe Base64
        url_safe_b64 = re.compile(
            r"(?:[A-Za-z0-9_-]{4}){10,}(?:[A-Za-z0-9_-]{2}==|[A-Za-z0-9_-]{3}=)?"
        )
        # Hex-encoded strings
...
class ContentSanitizer:
    ...
    @classmethod
    def sanitize(cls, content: str, mode: str = "neutralize", detect_injection: bool = True) -> str:
        """
    ...
    def _apply_paragraph_filter(cls, content: str) -> str:
        ...
        for para in paragraphs:
            ...
            if instruction_count >= 3:
                ...
                for pattern in instruction_indicators:
                    para = re.sub(
                        pattern, "[REDACTED PROMPT INJECTION ATTEMPT]", para, flags=re.IGNORECASE
                    )
                filtered_paragraphs.append(para)
            else:
                filtered_paragraphs.append(para)
    ...
    def sanitize_structured_data(cls, data):
        ...
        Every string value is sanitized, including URL-looking strings: a
        payload smuggled inside an ``https://...`` value must not skip filtering
        just because it parses as a URL (URLs are a common injection carrier).
        """
...
class Http(core.module.Module):
    ...
    settings = {
        ...
        "domain_blacklist": {"default": [], "description": "Forbid access to these domains"},
    }
    ...
    def _wrap_untrusted(self, content, source: str = "external_web") -> dict:
        ...
        # Detect injection patterns AND sanitize in one pass. We KEEP the
        # sanitized result from this call (previously it was computed and then
        # thrown away by an unconditional re-sanitize of the raw original, which
        # discarded the detection work and re-ran filtering on unsanitized text).
        if isinstance(content, str):
            ...
            # Non-string (dict/list/etc.): use the recursive structured sanitizer.
    ...
    def _truncate_for_safety(self, content: str) -> str:
        ...
        if len(content) > self.MAX_CONTENT_FOR_PROMPT:
            self._log(
                f"Content truncated from {len(content)} to {self.MAX_CONTENT_FOR_PROMPT} chars"
            )
    ...
    def _validate_output_safety(self, content: str) -> dict:
        """
        Best-effort heuristic check on already-sanitized output.

        This is advisory only, NOT a guarantee: it runs regex/ratio heuristics
        (residual injection patterns, large encoded blocks, high non-ASCII
        ratio) that can both miss real attacks and false-positive on legitimate
        content. The result is surfaced to callers (in ``security_metadata``);
        the caller decides what to do with a flagged response. It does not
        block or rewrite the content.

        Returns a dict with ``is_safe`` and any ``issues`` found.
        """
        issues = []
    ...
    def _check_domain_policy(self, hostname: str):
        ...
        if whitelist:
            allowed = any(hostname == d or hostname.endswith("." + d) for d in whitelist)
            if not allowed:
    ...
    def _resolve_and_validate(self, hostname: str):
        """
        Resolve a hostname to IP(s), validate ALL of them against the
        blocked-network list, and return one validated IP to CONNECT to.

        Returns (ok, error_message, pinned_ip). ``pinned_ip`` is a single
        validated address that the caller must dial directly (see
        ``_PinnedIPAdapter``) so that the address we validated here is the exact
        address the socket connects to. This is what actually defeats DNS
        rebinding / TOCTOU: without pinning, ``requests`` would perform its own
        second DNS lookup at connect time and could be handed a private/internal
        IP after this check passed. ``pinned_ip`` is ``None`` when the local-
        network guard is disabled (no pinning required).
        """
        ...
        # Preserve resolution order (getaddrinfo already ranks by preference)
        # while de-duplicating, so the pinned IP is a real, ordered candidate.
        ips = list(dict.fromkeys(info[4][0] for info in infos))
        if not ips:
            return False, "Hostname did not resolve to any address", None
        ...
        # Every resolved address is safe; pin the first for the actual connect.
    ...
    def _is_safe_url(self, url: str):
        ...
        Returns (ok, error_message, pinned_ip). ``pinned_ip`` is the validated
        IP address the caller must connect to directly (or ``None`` when the
        local-network guard is disabled or the URL was rejected).
        """
        try:
            parsed = urlparse(url)
        except Exception as e:
            return False, f"URL parse error: {e}", None

        # Scheme
        if parsed.scheme.lower() not in self.ALLOWED_SCHEMES:
            return False, f"Scheme not allowed: {parsed.scheme}", None

        if self.config.get("https_only") and parsed.scheme.lower() != "https":
            return False, "HTTPS-only mode is enabled", None
        ...
        if parsed.port and parsed.port in self.DANGEROUS_PORTS:
            return False, f"Port {parsed.port} is blocked", None
        ...
        if self.config.get("block_local_network_access") and hostname in (
            ...
            return False, "Blocked localhost access", None

        # DNS + resolved-IP validation (also returns the IP to pin the connect to)
    ...
    async def _make_request(self, func, url: str, **kwargs):
        ...
        passthrough = {
            ...
            if k
            not in ("headers", "allow_redirects", "stream", "verify", "timeout", "include_content")
        }
        ...
        try:
            for _hop in range(self.MAX_REDIRECTS + 1):
                ...
                # and obtain the validated IP to PIN this hop's connection to.
                ok, err, pinned_ip = self._is_safe_url(current_url)
                if not ok:
                    self._log(f"Blocked URL ({current_url}): {err}")
                    return self.result("URL blocked by security policy", False)

                # Pin the connection to the exact IP we just validated so that
                # requests cannot re-resolve the hostname to a different (e.g.
                # internal) address between the check and the connect (DNS
                # rebinding / TOCTOU). TLS SNI + cert validation stay bound to
                # the original hostname via the adapter. Re-pinned every hop.
                session = None
                if pinned_ip is not None:
                    session = requests.Session()
                    adapter = _PinnedIPAdapter(urlparse(current_url).hostname, pinned_ip)
                ...
                try:
                    ...
                    if session is not None:
                        resp = session.request(method, current_url, **request_kwargs)
                    else:
                        resp = func(current_url, **request_kwargs)
                    ...
                    # _build_response has fully consumed/closed the body by the
                    # time it returns, so it is safe to release the session's
                    # pooled connection here (and on redirect/continue).
                    if session is not None:
                        session.close()
    ...
    def _build_response(self, resp, include_content: bool):
        ...
        if include_content:
            ...
            raw_content = bytes(body[: self.MAX_CONTENT_SIZE]).decode(encoding, errors="replace")
            ...
            if "headers" in response:
                for header_name in list(response["headers"].keys()):
                    if header_name.lower() in ip_headers:
                        response["headers"][header_name] = self._censor_ip(
            ...
            # _validate_output_safety is a best-effort HEURISTIC (regex/ratio
            # based), not a guarantee. We do not blank the body on a hit (that
            # would false-positive on e.g. legitimate non-English or heavily
            # encoded pages); instead we surface an explicit, honest flag that
            # downstream callers can gate on, alongside the full result already
            # stored in validation_result.
            if not validation["is_safe"]:
                self._log(f"Output validation flagged {resp.url}: {validation['issues']}")
                response["security_metadata"]["output_validation_failed"] = True
    ...
    async def get(self, url: str, headers: dict = None, params: dict = None):
        return await self._make_request(
            requests.get, url, params=params, headers=headers, include_content=True
        )
    ...
    async def post(self, url: str, headers: dict = None, data: dict = None, json: dict = None):
        ...
        return await self._make_request(
            requests.post, url, data=data, json=json, headers=headers, include_content=True
        )

    async def head(self, url: str, params: dict = None, headers: dict = None):
        return await self._make_request(
            requests.head, url, params=params, headers=headers, include_content=False
        )

    async def options(self, url: str, params: dict = None, headers: dict = None):
        return await self._make_request(
            requests.options, url, params=params, headers=headers, include_content=False
        )

    async def put(self, url: str, data: dict = None, headers: dict = None):
        return await self._make_request(
            requests.put, url, data=data, headers=headers, include_content=True
        )

    async def patch(self, url: str, data: dict = None, headers: dict = None):
        return await self._make_request(
            requests.patch, url, data=data, headers=headers, include_content=True
        )

    async def delete(self, url: str, params: dict = None, headers: dict = None):
        return await self._make_request(
            requests.delete, url, params=params, headers=headers, include_content=True
        )
