...
class Webui(core.channel.Channel):
    ...
    settings = {
        ...
        "custom_host": {"default": None, "depends": {"network_mode": "custom"}},
        "port": {
        ...
        "show_unsafe_settings": {
            "description": "Whether to show unsafe settings. This setting has to be manually toggled via the Settings modal (gear icon), the `/config` command, or by editing the config file, because if you want access to the unsafe features, you hopefully know what you're doing!",
            "default": False,
            "unsafe": True,
        },
        "require_login": {
        ...
        "username": {"default": "admin", "depends": "require_login"},
        "password": {"default": "admin", "depends": "require_login"},
        "login_lifetime": {
        ...
        "session_secret": {
            "description": "Secret key used to sign session cookies. Generated automatically on first run and persisted here - keep it private and do NOT share it. Leave empty to have a fresh random secret generated.",
            "default": None,
            "unsafe": True,
        },
        "https_only": {
            "description": "Mark the session cookie as Secure so it is only ever sent over HTTPS. Enable this when the WebUI is served over HTTPS (for example behind a TLS-terminating reverse proxy).",
            "default": False,
            "depends": "require_login",
        },
        "trust_proxy": {
            "description": "Enable ONLY when running behind a trusted reverse proxy. When on, forwarded headers (X-Forwarded-For) from the configured proxy IPs are trusted to determine the real client IP, which the login rate-limiter relies on. Leave off otherwise, or forwarded headers could be spoofed to bypass rate-limiting.",
            "default": False,
        },
        "trusted_proxy_ips": {
            "description": "Comma-separated list of proxy IP addresses whose forwarded headers are trusted. Only used when `trust_proxy` is enabled.",
            "default": "127.0.0.1",
            "depends": "trust_proxy",
        },
    }
    ...
    def _get_or_create_session_secret(self) -> str:
        """Return the persisted session-signing secret, generating and saving a
        cryptographically-random one on first run. Never falls back to a shared
        hardcoded constant, so signed session cookies cannot be forged."""
        secret = self.config.get("session_secret")
        if not secret:
            secret = secrets.token_urlsafe(48)
            # persist it via the same config mechanism as every other setting
            self.config["session_secret"] = secret
        return secret
    ...
    async def on_ready(self):
        ...
        # warn loudly if login is enabled but the credentials are still the
        # insecure built-in defaults - do NOT silently accept them
        if (
            self.config.get("require_login", False)
            and self.username == "admin"
            and self.password == "admin"
        ):
            exposed = network_mode in ("internet", "custom")
            self.log(
                "webui",
                "SECURITY WARNING: WebUI login is enabled but username/password are still the default 'admin'/'admin'. "
                "Change them in this channel's settings before use"
                + (
                    " - the WebUI is bound to a non-local address and is exposed with default credentials!"
                    if exposed
                    else "."
                ),
            )
    ...
    async def run(self):
        ...
        # Only trust forwarded headers (X-Forwarded-For etc) when explicitly
        # configured to run behind a trusted proxy. Otherwise a client could
        # spoof its IP and bypass the login rate-limiter (which keys on
        # request.client.host). Default: trust none.
        trust_proxy = self.config.get("trust_proxy", False)
        forwarded_allow_ips = (
            self.config.get("trusted_proxy_ips", "127.0.0.1") if trust_proxy else []
        )
        ...
        config = uvicorn.Config(
            ...
            host=self.host,
            port=self.port,
            # only trust proxy-set forwarded headers behind a configured trusted proxy
    ...
    async def on_push(self, message):
        await self.websocket_manager.broadcast({"type": "push", "content": message})
    ...
    def on_log(self, category, message):
        ...
        log_message = {"type": "log", "category": category, "message": message}
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(self.websocket_manager.broadcast(log_message))
        except RuntimeError:
            # No event loop running - we can't broadcast right now (scheduling a
            # coroutine without a running loop would raise). The log is already
            # buffered above, so it will still be served via /api/system/logs.
            pass
...
def verify_session_cookie(secret: str, cookie: str, max_age) -> bool:
    """Verify a Starlette SessionMiddleware-signed session cookie and return
    whether it represents an authenticated session.

    Mirrors exactly how SessionMiddleware loads a session: it unsigns the value
    with an itsdangerous TimestampSigner keyed on the same secret, then base64-
    decodes the JSON payload. A garbage/forged/expired cookie fails to unsign
    and is rejected."""
    if not cookie:
        return False
    signer = itsdangerous.TimestampSigner(str(secret))
    try:
        data = signer.unsign(cookie.encode("utf-8"), max_age=max_age)
...
# ---------------------------------------------------------------------------
# Secret redaction for the settings APIs
# ---------------------------------------------------------------------------
...
# "auth" matches auth_type / auth_header_name, which are not secrets.
_SENSITIVE_KEY_EXCEPTIONS = frozenset({"auth_type", "auth_header_name"})
...
def redact_config_secrets(data):
    ...
    Same key-name approach as modules/config.py so the settings UI never
    receives API keys, passwords, session secrets, etc. in plaintext.
    Non-string values (booleans like api_key_required) are left intact."""
...
def _match_list_item(item, current_list, index):
    """Match a list-of-dicts item to the stored config by `name`, else by index."""
    if not isinstance(current_list, list):
        return None
    if isinstance(item, dict) and item.get("name"):
        name = item.get("name")
        named = [c for c in current_list if isinstance(c, dict) and c.get("name") == name]
        if len(named) == 1:
            return named[0]
        if index < len(current_list):
            cur = current_list[index]
            if isinstance(cur, dict) and cur.get("name") == name:
                return cur
            if named:
                return named[0]
    if index < len(current_list):
...
def restore_redacted_secrets(incoming, current):
    ...
    redacted placeholder (or left blank) is restored from the stored `current`
    config.

    Recurses through dicts and lists of dicts (matching by index or `name`) so
    nested secrets such as MCP `servers[].api_key` are not overwritten with
    "****". A field the user actually edited no longer equals the placeholder,
    so their new value is kept as-is."""
    if isinstance(incoming, dict):
        ...
        for k, v in incoming.items():
            ...
            if _is_sensitive_key(k) and v == _REDACTED_PLACEHOLDER:
                result[k] = cur
            elif _is_sensitive_key(k) and v in ("", None) and cur not in ("", None):
                result[k] = cur
            elif isinstance(v, dict):
                result[k] = restore_redacted_secrets(v, cur if isinstance(cur, dict) else {})
            elif isinstance(v, list):
                result[k] = restore_redacted_secrets(v, cur if isinstance(cur, list) else [])
            else:
                result[k] = v
        return result
    if isinstance(incoming, list):
        ...
        return [
            restore_redacted_secrets(item, _match_list_item(item, current_list, i))
...
async def create_fastapi(channel):
    ...
    async def auth_middleware(request: fastapi.Request, call_next):
        ...
        if request.url.path in ["/login", "/logout"] or str(request.url.path).startswith(
            "/assets/"
        ):
            return await call_next(request)
    ...
    # per-install random secret, generated + persisted on first run (never a
    # shared hardcoded default, so signed session cookies cannot be forged).
    # stash it on the channel so the WebSocket handler can verify cookies the
    # same way SessionMiddleware does.
    session_secret = channel._get_or_create_session_secret()
    channel.session_secret = session_secret
    channel.session_max_age = session_lifetime_days * 86400
    app.add_middleware(
        starlette.middleware.sessions.SessionMiddleware,
        secret_key=session_secret,
        max_age=channel.session_max_age,
        # strict SameSite is fine for a single-user tool and blocks the session
        # cookie from riding cross-site requests (a lightweight CSRF guard).
        same_site="strict",
        # mark the cookie Secure when served over HTTPS (configurable)
        https_only=channel.config.get("https_only", False),
    )

    # serve asset files (formerly /static) using fastAPI's mount()
    app.mount(
        "/assets", fastapi.staticfiles.StaticFiles(directory=channel.assets_path), name="assets"
    )
    ...
    async def root(request: fastapi.Request):
        ...
        js_files = get_recursive_assets(
            os.path.join(channel.assets_path, "js"),
            "js",
            skip=["init.js", "stores", "libs", "utils"],
        )
    ...
    # old standalone /config page is gone; gear-icon settings modal is the
    # one settings surface. keep the URL so bookmarks do not 404.
    @app.get("/config")
    async def config_page():
        """Redirects to the main WebUI. Settings live in the gear-icon modal."""
    ...
    async def search_chats(request: fastapi.Request):
        ...
        if category and category != "general":
            ...
            results = [
                r for r in results if not r.get("category") or r.get("category") == "general"
            ]
    ...
    async def settings_load():
        """Returns the core's config object as JSON, with secrets redacted by
        key name (same approach as modules/config.py). API keys, passwords and
        the session secret are never sent to the browser in plaintext."""
    ...
    async def get_module_info():
        ...
        for section_name, section_data in core_structure.items():
            if section_name not in module_info:
    ...
    async def settings_save(request: fastapi.Request):
        """Saves config data to the backend. Accepts a structure that reflects
        core.config.config exactly (check /api/settings/load). Before writing,
        any secret still holding the redacted placeholder (or left blank) is
        restored from the stored config, including secrets nested in lists of
        dicts such as MCP server api_key fields."""
        data = await request.json()
    ...
    async def get_data():
        ...
        data = {"max_context": core.config.get("api", "max_context")}
    ...
    async def get_theme(family_name: str):
        ...
        try:
            ...
            return api_result(f"Failed to load theme: {e!s}", success=False)
    ...
    async def service_worker():
        ...
        # assets are served from /assets/ (mounted on channel.assets_path), not
        # /static/ - walk the real asset dir so the precache list isn't empty
        assets_base = channel.assets_path
    ...
    async def websocket_endpoint(websocket: fastapi.WebSocket):
        ...
        if channel.config.get("require_login", False):
            # Verify the signed session cookie the same way SessionMiddleware
            # does - a merely-present cookie is not enough; it must be a valid,
            # non-expired signature carrying authenticated=true. A missing,
            # garbage or forged cookie is rejected.
            session_cookie = websocket.cookies.get("session")
            authenticated = verify_session_cookie(
                channel.session_secret, session_cookie, channel.session_max_age
            )
                ...
                try:
                    ...
                    match msg_type:
                        ...
                        case "switch_chat":
                            ...
                            if new_chat_id:
                                if (
                                    ws_mgr.active_stream_task
                                    and not ws_mgr.active_stream_task.done()
                                ):
                                ...
                                try:
                                    ...
                                    await ws_mgr.broadcast(
                                        {"type": "error", "content": f"Failed to load chat: {e}"}
                                    )
                        ...
                        case "new_chat":
                            ...
                            await ws_mgr.broadcast(
                                {"type": "chat_switched", "chat_id": new_id, "buffer": []}
                            )
                        case "chat_delete":
                        ...
                        case "user_message":
                            ...
                            if files_data:
                                files_dict = {
                                    f["name"]: base64.b64decode(f["data"]) for f in files_data
                                }

                            chat_id = channel.context.chat.get("id") or "default"
                            await ws_mgr.start_stream(
                                channel, chat_id, message=text, files=files_dict
                            )
                        case "message_edit":
                        ...
                        case "message_regenerate":
                            ...
                            if index is not None and channel:
                                last_user_message_index = (
                                    await channel.context.chat.messages.get_last_message_with_role(
                                        "user", cutoff_index=index
                                    )
                                )
                                ...
                                user_message = await channel.context.chat.messages.get(
                                    last_user_message_index
                                )
                                ...
                                await channel.context.chat.messages.delete_from(
                                    max(0, last_user_message_index)
                                )

                                await ws_mgr.broadcast({"type": "sync"})
                                await ws_mgr.start_stream(
                                    channel,
                                    channel.context.chat.get("id"),
                                    user_message.get("content"),
                                )
                        case _:
                            channel.log(
                                channel.name, f"Unknown websocket command received: {msg_type}"
                            )
                    ...
                    match token_type:
                        case "user_message":
                            try:
                                ...
                                self.channel.log(
                                    self.channel.name,
                                    f"error sending user message: {core.detail_error(e)}",
                                )
                                return
                        case "error":
                            # for an error, just force a chat reload so that it shows up (core/channel takes care of adding it to context)
                            await self.broadcast({"type": "user_message_confirmed", "index": index})
                            await self.broadcast({"type": "sync"})
                            return
                        case _:
                            if not user_message_confirmed:
                                user_message_confirmed = True
                                await self.broadcast(
                                    {"type": "user_message_confirmed", "index": index}
                                )

                            await self.broadcast({"type": "token", "content": token})

                elif partial.get("type") == "turn":
                    await self.broadcast({"type": "turn_stream", "turn": partial.get("content")})
        finally:
            # always finalize the stream, no matter what
            await self.broadcast({"type": "stream_complete"})
    ...
    async def start_stream(self, channel, chat_id: str, message: str, files: list = None):
        ...
        try:
            self.active_stream_task = asyncio.create_task(
                self._stream_task(message, next_index, files=files)
            )
